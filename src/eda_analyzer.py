...
def calculate_pole_win_probability(df):
    ...
    logger.info("Đang phân tích Q1: Xác suất thắng từ Pole...")
    
    pole_starts = df[df['grid'] == 1]
    pole_wins = pole_starts[pole_starts['positionOrder'] == 1]
    
    if len(pole_starts) == 0:
        logger.warning("Không có dữ liệu xuất phát ở vị trí Pole.")
        return
...
def compare_eras(df):
    ...
    logger = logging.getLogger("F1_System")
    logger.info("Đang phân tích Q2: So sánh nhóm xuất phát (Từ 2010 đến nay)...")
...
def analyze_circuit_correlation(df):
    ...
    logger = logging.getLogger("F1_System")
    logger.info("Đang phân tích Q3: Tương quan vị trí (Monaco vs Monza)...")
    ...
    logger.info(f"Đã lưu bảng tương quan các trường đua tại: {metrics_path}")
    ...
    try:
        ...
        logger.warning("Không đủ dữ liệu cho Monaco hoặc Monza để so sánh.")
    print("-"*50)

def plot_advantage_decay(df):
    """Vẽ và lưu 2 biểu đồ thể hiện sự suy giảm lợi thế (Advantage Decay)."""
    logger = logging.getLogger("F1_System")
    logger.info("Đang phân tích Q4: Vẽ biểu đồ Advantage Decay...")
    ...
    plt.title('Sự suy giảm Xác suất đạt Podium theo Vị trí Xuất phát', fontsize=14, fontweight='bold')
    plt.ylabel('Xác suất (0 - 1)', fontsize=12)
    plt.xlabel('Vị trí Grid', fontsize=12)
    ...
    plt.title('Vị trí Về đích Trung bình theo Vị trí Xuất phát', fontsize=14, fontweight='bold')
    plt.ylabel('Thứ hạng về đích (Càng thấp càng tốt)', fontsize=12)
    plt.xlabel('Vị trí Grid', fontsize=12)
    ...
    logger.info(f"Đã xuất bản thành công 2 biểu đồ vào thư mục: {PLOT_DIR}")

def perform_eda_analysis(df):
    """Hàm điều phối chạy toàn bộ phân tích EDA."""
    logger = logging.getLogger("F1_System")
    logger.info("=== BẮT ĐẦU PHÂN TÍCH KHÁM PHÁ (EDA) ===")
    ...
    logger.info("Hoàn thành module EDA Analyzer.")
