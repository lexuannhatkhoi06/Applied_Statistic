...
def enforce_determinism():
    ...
    try:
        ...
        logger.info(f"Đã kích hoạt chế độ Đơn luồng (Op Determinism) cho TensorFlow.")
    except AttributeError:
        # Đề phòng trường hợp máy bạn cài bản TensorFlow quá cũ (< 2.8) chưa có hàm này
        logger.warning("Phiên bản TensorFlow hiện tại không hỗ trợ enable_op_determinism(). Hãy cập nhật TF >= 2.8 nếu kết quả bị lệch.")

    logger.info(f"Đã khóa toàn bộ tính ngẫu nhiên hệ thống với Seed = {RANDOM_SEED}")